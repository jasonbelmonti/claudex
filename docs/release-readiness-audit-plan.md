# 3.0.0 Release Readiness Audit Plan

This plan defines the pre-release audit for the 3.0.0 semver-major
`@jasonbelmonti/claudex` release.

## Audit Objective

- Artifact under review: `claudex` on `main` at or after merge commit `7c546bb`.
...
- Depth: targeted release-readiness audit with deep review for package contract,
  provider runtime behavior, Copilot beta-provider boundaries, and release
  automation.
...
- Provider runtime surface: `src/providers/claudex/**`,
  `src/providers/claude/**`, `src/providers/codex/**`, and
...
- Validation commands: `npm run check`, `npm test`, `npm run test:coverage`,
  ...
  `npm run test:smoke`.
- Release target: package version `3.0.0`, tag `v3.0.0`.
...
- Core behaviors to validate:
  ...
  - README, release notes, and consumer docs match the package name, Node-only
    runtime policy, Copilot preview posture, and ESM-only module contract.
...
- Primary files or directories: `src/providers/claudex/**`,
  ...
  `src/providers/copilot/**`, `test/providers/**`, and `test/contract/**`.
- Core behaviors to validate:
  ...
  - `createSession`, `resumeSession`, `run`, and `runStreamed` preserve the
    normalized contract across Claude, Codex, and opt-in Copilot.
...
- In scope: Claude, Codex, Copilot, and Claudex provider adapters, sessions,
  readiness, event/result mapping, provider option merging, and provider tests.
...
- Primary files or directories: `package.json`, `package-lock.json`,
  ...
  `scripts/package-check/**`, `.github/workflows/ci.yml`,
  `.github/workflows/release.yml`, and `docs/release-notes/3.0.0.md`.
- Core behaviors to validate:
...
- Deliverables:
  ...
  - Confirmation that `v3.0.0` can proceed once validation passes.
...
- Primary files or directories: `test/**`, `docs/node-only-migration.md`,
  `docs/normalized-sdk-plan.md`, `docs/capability-matrix.md`,
  `docs/release-notes/3.0.0.md`, and any local audit notes provided for the
  release.
- Core behaviors to validate:
...
- Key failure modes or regressions:
  ...
  - Missing final validation evidence for `v3.0.0`.
...
- Node-only release posture: Groups 1, 4, and 5. Confirm no Bun-only runtime,
  tooling, or documentation assumptions remain in the supported path, and that
  Node 20 is the package floor.
...
- Authenticated Claude, Codex, and Copilot smoke tests require local provider
  availability and may need to be recorded as optional release evidence.
- The exact semver-major target is `3.0.0`; the tag must be `v3.0.0` and must
  point at a commit whose package metadata also says `3.0.0`.

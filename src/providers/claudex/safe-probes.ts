...
function freezeProbe(probe: {
  ...
  checks: SafeProviderProbeCheck[];
}): SafeProviderProbe {
  const checks = probe.checks.map((check) => Object.freeze({ ...check }));
...
function safeProbeText(value: unknown, maximumLength: number): string {
  ...
  if (
    SENSITIVE_SUMMARY_LABEL.test(text) ||
    CREDENTIAL_SHAPE.test(text)
  ) {

...
export function resolveCopilotSdkOptions(
  ...
  const executable = findCopilotOnPath({
    env: mergeEnvironments(ambientEnv, configuredEnv, platform),
...
function findCopilotOnPath(params: {
  ...
  const pathValue = getEnvironmentValue(params.env, "PATH", params.platform);
  if (!pathValue) {
    return undefined;
  }

...
import { resolveCopilotSdkOptions } from "../../../src/providers/copilot/cli-path-resolution.js";

test("Copilot CLI resolution preserves an explicit connection path", () => {
  const options = resolveCopilotSdkOptions(
    { connection: { kind: "stdio", path: "/explicit/copilot" } },
    {
      env: {
...
test("Copilot CLI resolution preserves SDK platform fallback when no CLI is found", () => {
